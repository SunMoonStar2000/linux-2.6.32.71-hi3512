...
unsigned long hisilicon_irq_prio_masks[8] = {
	...
	(1<<INTNR_DSU),
	(1<<INTNR_ETH) | (1<<INTNR_PCI) | (1<<INTNR_MMC),
	~0,
	0,
};
...
#define HIL_AMBA_DEVICE(name,initname,base,platdata)		\
...
static struct map_desc hisilicon_io_desc[] __initdata = {
	...
	REG_IOMAP( SIO_1 ),
	REG_IOMAP( USB11 ),
	REG_IOMAP( ExAHB_ARBITER ),
	REG_IOMAP( DMAMEM_AHB_ARBITER ),
	REG_IOMAP( PCI_BR ),
	REG_IOMAP( PCI_IO ),
#if 1
	{
		.virtual        =REG_BASE_PCI_NP_VIRT,
		.pfn            = __phys_to_pfn(REG_BASE_PCI_NP),
		.length         = REG_PCI_NP_IOSIZE,
		.type           = MT_DEVICE
	},{
		.virtual        =REG_BASE_PCI_PF_VIRT,
		.pfn            = __phys_to_pfn(REG_BASE_PCI_PF),
		.length         = REG_PCI_PF_IOSIZE,
		.type           = MT_DEVICE
	}
#endif
};
...
/* HI-hi3511v100 TIMER0 USING BUS CLK */
...
#if 0 
		tmp = readl(IO_ADDRESS(REG_BASE_SCTL + REG_SC_CTRL));
		/* FIXME: hard-code!!! */
		set_bit(16, &tmp);
		clear_bit(15, &tmp);
		writel(tmp, IO_ADDRESS(REG_BASE_SCTL + REG_SC_CTRL));
#endif
...
static cycle_t hisi_get_cycles(struct clocksource *cs)
{
	//edb_trace(0);
...
MACHINE_START(HI3511V100, "Hi3511v100")
