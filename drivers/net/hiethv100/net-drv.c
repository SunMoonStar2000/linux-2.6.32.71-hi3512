...
static int hieth_net_open(struct net_device *dev)
{
	...
	ret = request_irq(dev->irq, hieth_net_isr, IRQF_SHARED, dev->name, dev);
	if(ret) {
...
static const struct net_device_ops hieth100_netdev_ops = {
	...
	.ndo_set_multicast_list	= hieth_net_set_multicast_list,
...
static int __init hieth_dev_probe_init(struct net_device *dev)
{
	...
	dev->netdev_ops	= &hieth100_netdev_ops;
...
static int __devinit hieth_dev_probe(struct platform_device *pdev)
{
	int ret = -1;
	...
	printk(KERN_INFO "hieth_dev_probe.\n");		
	...
	platform_set_drvdata(platdev, netdev);
	ld = netdev_priv(netdev);
	ld->pdev = pdev;
	ld->ndev = netdev;

	if(hieth_mdiobus_probe(&platdev->dev)) {
		hieth_error("hieth_mdiobus_probe failed!");
		goto _error_exit;
	}
	...
	ld->irq = irqres->start;
#if 0 //lcy
	printk(KERN_INFO "ld->iobase:%x,ld->iobase_phys:%x, ld->irq:%x.\n",
			ld->iobase, ld->iobase_phys, ld->irq);		
#endif

	netdev->irq = ld->irq;

	SET_MODULE_OWNER(netdev);
	SET_NETDEV_DEV(netdev, &platdev->dev);
        dev_set_drvdata(&platdev->dev, netdev);
...
static int hieth_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver hieth_dev_driver ={
	.driver = {
		.name = HIETH_DEV_NAME,
		.owner = THIS_MODULE,
	},
	.probe = hieth_dev_probe,
	.remove = __devexit_p(hieth_dev_remove),
};
static int __init hieth_init(void)
{
	...
	ret = platform_driver_register(&hieth_dev_driver);
	if(ret) {
	...
	if(hieth_wq ==NULL) {
		...
		platform_driver_unregister(&hieth_dev_driver);
#if 0 //lcy
		hieth_mdiobus_driver_exit();
#endif
		hieth_mm_exit();
