...
{
	int val = 0;
	hieth_assert( (!(phy_id & (~0x1F))) && (!(regnum & (~0x1F))) );
	...
	mdio_start_phyread(ld, phy_id, regnum);
...
error_exit:
	...
	hieth_trace(4, "phy_id = %d, regnum = %d, val = 0x%04x", phy_id, regnum, val);
...
int hieth_mdio_write(struct hieth_mdio_local *ld, int phy_id, int regnum, int val)
{
	...
	hieth_trace(4, "phy_id = %d, regnum = %d", phy_id, regnum);
	...
	mdio_phywrite(ld, phy_id, regnum, val);
