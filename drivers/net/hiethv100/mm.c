...
static struct kmem_cache *hieth_frame_cache;
		...
		for(i=0; i<nr_frames; i++) {
			...
			if(mode ==1) {
				...
				if(hieth_recv_queue_pushfd(ld, &fd) !=0 && 
						hieth_recv_queue_pushfd_rc(ld, &fd) !=0)
					break;
				dma_cache_maint(fd.priv_data, HIETH_MAX_FRAME_SIZE, DMA_FROM_DEVICE);
			} else {
				hieth_trace(3, "unsupport mode=%d", mode);
				break;
			}
...
static int hieth_create_hwq_kmem_cache(struct net_device *dev, int nr, int mode)
{
	...
	for(i=0; i<nr; i++) {
		...
		mb_virt = kmem_cache_alloc(hieth_frame_cache, GFP_KERNEL);
		if(mb_virt ==NULL) {
...
int hieth_recvq_skb_feed(struct net_device *dev, int nr, unsigned long prio)
{
	...
	for(; nr!=0; nr--) {
		...
		dma_cache_maint(skb->data, HIETH_MAX_FRAME_SIZE, DMA_FROM_DEVICE);
...
{
	kmem_cache_destroy(hieth_frame_cache);
}
