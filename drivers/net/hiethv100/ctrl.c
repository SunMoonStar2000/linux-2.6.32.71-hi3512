...
//added by wzh for multicast filter 2009-3-24
void hieth_set_multicast_filter(struct hieth_netdev_local *ld, unsigned short flags)
{
	if (flags&IFF_ALLMULTI)
	{
		hieth_writel_bits(ld, 1, GLB_MACFILT_BEHAVE, BITS_MULTI_CTRL);
		hieth_writel_bits(ld, 0, GLB_MACFILT_BEHAVE, BITS_MAC1_FILT_ENA);
		hieth_writel_bits(ld, 0, GLB_MACFILT_BEHAVE, BITS_MAC0_FILT_ENA);
	}
	else
	{
		hieth_writel_bits(ld, 0, GLB_MACFILT_BEHAVE, BITS_MULTI_CTRL);
	}
	return ;
}

int mc_group_count = 0;

int eth_set_mac0_filter(struct hieth_netdev_local *ld, struct dev_mc_list *mc_list)
{
	unsigned long reg;
	reg = mc_list->dmi_addr[1] | (mc_list->dmi_addr[0] <<8);
	hieth_writel(ld, reg, GLB_MACFILT_0_H2);
	reg = mc_list->dmi_addr[5] | (mc_list->dmi_addr[4]<<8) | (mc_list->dmi_addr[3]<<16) | (mc_list->dmi_addr[2]<<24);
	hieth_writel(ld, reg, GLB_MACFILT_0_L4);
	hieth_writel_bits(ld, 1, GLB_MACFILT_BEHAVE, BITS_MAC0_FILT_ENA);
	return 0;
}

int eth_set_mac1_filter(struct hieth_netdev_local *ld, struct dev_mc_list *mc_list)
{
	unsigned long reg;
	reg = mc_list->dmi_addr[1] | (mc_list->dmi_addr[0] <<8);
	hieth_writel(ld, reg, GLB_MACFILT_1_H2);
	reg = mc_list->dmi_addr[5] | (mc_list->dmi_addr[4]<<8) | (mc_list->dmi_addr[3]<<16) | (mc_list->dmi_addr[2]<<24);
	hieth_writel(ld, reg, GLB_MACFILT_1_L4);
	hieth_writel_bits(ld, 1, GLB_MACFILT_BEHAVE, BITS_MAC1_FILT_ENA);
	return 0;
}

int hieth_clear_mac_filter(struct hieth_netdev_local *ld)
{
	hieth_writel(ld, 0, GLB_MACFILT_0_H2);
	hieth_writel(ld, 0, GLB_MACFILT_0_L4);
	hieth_writel(ld, 0, GLB_MACFILT_1_H2);
	hieth_writel(ld, 0, GLB_MACFILT_1_L4);
	return 0;
}

int hieth_set_mc_mac_filter(struct hieth_netdev_local *ld, struct dev_mc_list *mc_list, int mc_count, 
												unsigned short cur_multi_flag, unsigned short pre_multi_flag)
{
	if( mc_count <= 0 )
	{
		hieth_writel_bits(ld, 0, GLB_MACFILT_BEHAVE, BITS_MAC0_FILT_ENA);
	...
	if(1 == mc_count )
	{
		if(mc_count > mc_group_count)
		{
			eth_set_mac0_filter(ld, mc_list);
		}
		hieth_writel(ld, 0, GLB_MACFILT_1_H2);
		hieth_writel(ld, 0, GLB_MACFILT_1_L4);
		mc_group_count = mc_count;
	...
	if(2 == mc_count )
	{
		eth_set_mac1_filter(ld, mc_list);

		if((cur_multi_flag != pre_multi_flag) && mc_list->next)
		{
			eth_set_mac0_filter(ld, mc_list->next);
		}
		mc_group_count = mc_count;
	}
