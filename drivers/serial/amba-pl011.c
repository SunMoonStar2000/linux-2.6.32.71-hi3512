...
#define SCTL_RTS_CR	IO_ADDRESS(REG_BASE_SCTL + REG_SC_PERCTRL4)
#define RTS_SET_HIGH(reg)	((reg) | (1 << 26))
#define RTS_SET_LOW(reg)	((reg) & (~(1 << 26)))
#define RS485_PORT	1 
...
static void pl011_stop_tx(struct uart_port *port)
{
	...
	if(uap->port.line == RS485_PORT) {
		unsigned long reg;

		while(!pl01x_tx_empty(port));

		reg = readl(SCTL_RTS_CR);
		writel(RTS_SET_LOW(reg), SCTL_RTS_CR);
	}
...
static void pl011_start_tx(struct uart_port *port)
{
	...
	if(uap->port.line == RS485_PORT) {
		...
		reg = readl(SCTL_RTS_CR);
		writel( RTS_SET_HIGH(reg), SCTL_RTS_CR);
	}
}
